...
def create_small_isic_dataset(source_dir, target_dir, num_samples=10, seed=42, force=False):
    """
    Create a small subset of the ISIC-2017 dataset for quick testing.
    ...
    for split in splits:
        ...
        if len(valid_img_files) > num_samples:
            selected_img_files = random.sample(valid_img_files, num_samples)
        else:
            selected_img_files = valid_img_files
            print(f"    Warning: Only {len(valid_img_files)} files available with masks in {split}")
        ...
        # Copy selected files
        for i, img_file in enumerate(selected_img_files, 1):
            ...
            # Print progress
            print(f"    Copied {i}/{len(selected_img_files)}: {img_file} and its mask")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create a small subset of the ISIC-2017 dataset for testing")
    parser.add_argument("--source", type=str, default="./dataset/isic-2017-kaggle", help="Source dataset directory")
    parser.add_argument("--target", type=str, default="./small_isic_dataset", help="Target directory for small dataset")
    parser.add_argument("--samples", type=int, default=10, help="Number of samples per split")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--force", action="store_true", help="Force overwrite of target directory without asking")
    
    args = parser.parse_args()
    
    create_small_isic_dataset(args.source, args.target, args.samples, args.seed, args.force)
