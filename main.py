...
from dataloader import GEN_DATA_LISTS, Cityscape, ISICDataset
from data_utils import collate, pallet_cityscape, pallet_isic
...
# Select appropriate palette based on dataset
if config['num_classes'] == 2:  # ISIC dataset
    ...
    palette = pallet_cityscape
    g2c = lambda x : gray2color(x, use_pallet='cityscape', custom_pallet=palette)
...
# Use appropriate dataset class based on number of classes
if config['num_classes'] == 2:  # ISIC dataset
    DatasetClass = ISICDataset
else:  # Cityscapes dataset
    DatasetClass = Cityscape

train_data = DatasetClass(train_paths[0], train_paths[1], config['img_height'], config['img_width'],
...
val_data = DatasetClass(val_paths[0], val_paths[1], config['img_height'], config['img_width'],
...
model = SegNext(num_classes=config['num_classes'], in_channnels=3, embed_dims=[32, 64, 460, 256],
                ...
                dec_outChannels=256, drop_path=float(config['stochastic_drop_path']),
                config=config)
...
#%%
# Add if __name__ == '__main__' guard to prevent multiprocessing issues
